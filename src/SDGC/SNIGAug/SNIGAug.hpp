...
void SNIGAug::_input_alloc() {
  ...
  std::vector<bool*> All32{2, nullptr};
  {
    for(int buff = 0; buff < 2; ++buff) {
      checkCuda(cudaMalloc(&All32[buff], Base::_num_inputs * Base::_num_secs * sizeof(bool)));
      checkCuda(cudaMemset(All32[buff], 0, Base::_num_inputs * Base::_num_secs * sizeof(bool)));
    }
