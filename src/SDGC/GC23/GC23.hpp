...
{
  std::cout<<("Constructing GC23 method......")<<std::endl;
}


void GC23::_dense_reorder(std::vector<std::vector<float>> &input, Reorder &reorder_class) {
...
int GC23::_infer(
    ...
    Safe_Call(cudaMallocManaged((void**)&A_d, sizeof(float) * neuron * batch));
    Safe_Call(cudaMemcpy(A_d, A, sizeof(float) * neuron * batch, cudaMemcpyHostToDevice));


    Safe_Call(cudaMallocManaged((void**)&A_T, sizeof(float) * neuron * batch));
    Safe_Call(cudaMemset(A_T, 0, sizeof(float) * neuron * batch));

    Safe_Call(cudaMallocManaged((void**)&C_d, sizeof(float) * neuron * batch));
    Safe_Call(cudaMemset(C_d, 0, sizeof(float) * neuron * batch));

    Safe_Call(cudaMallocManaged((void**)&active_d, sizeof(int) * batch));
    Safe_Call(cudaMallocManaged((void**)&category_d, sizeof(int) * batch));
    Safe_Call(cudaMallocManaged((void**)&old_to_new_map_d, sizeof(int) * batch));

    for(int l = 0; l < layer; ++l) {
        Safe_Call(cudaMallocManaged((void**)&(B_d[l]), sizeof(float) * weight[l].size()));
        Safe_Call(cudaMemcpy(B_d[l], B[l], sizeof(float) * weight[l].size(), cudaMemcpyHostToDevice));

        Safe_Call(cudaMallocManaged((void**)&(index_d[l]), sizeof(float) * row_access[l].size()));
    ...
    tf::Task cudaflows = (taskflow.emplace_on([&](tf::cudaFlow& cf){
        std::vector<tf::cudaTask> active_set;
        std::vector<tf::cudaTask> pre_infers;
        tf::cudaTask conversion;
        std::vector<tf::cudaTask> post_infers;
        tf::cudaTask recovery_task;
        std::vector<tf::cudaTask> hsscan_task;
        std::vector<tf::cudaTask> hsintgrl_task;
        std::vector<tf::cudaTask> feature_task;
 
        active_set.reserve(layer);
        pre_infers.reserve(threshold);
        post_infers.reserve(layer-threshold);
        hsscan_task.reserve(layer-threshold);
        hsintgrl_task.reserve(layer-threshold);
        feature_task.reserve(layer-threshold);

        for(int l = 0; l < layer; ++l) {
            //tasks of cudaflow
            active_set.emplace_back(cf.memset(
                active_d,
                0,
                sizeof(int) * batch
            ).name("active_set"));
        }
        pre_infers.emplace_back(cf.kernel(
            grid((this_round_batch + MINIBATCH - 1)/ MINIBATCH, (neuron + 112 - 1) / 112),
            128,
            sizeof(float) * (MINIBATCH * (128 + 16)),
            n16384l1_kernel_GC23,
            A_d, B_d[l], C_d, index_d[l], active_d, this_round_batch, neuron, bias
        ).name("Inference"));
    ...
    for(int l = 0; l < threshold; ++l) {
            ...
            if(l == threshold) {
                ...
                env.event_stop_record("GC23");
                float time = env.get_event_time("GC23"); 
                all_time += time;
                pre_conv_time += time;
                env.event_start_record("GC23");
                ...
                for (int idx = 0; idx < transpose_batch; idx++) {
                    if (ne_record[idx] == true) {
                        ...
                        //     if (A_d[idx*neuron+i-1]&&A_d[idx*neuron+i] == 1) {
                        //         if (end) {
                        //             segments++;
                        //             segmentIdx.push_back(i);
                        //             end = false;
                        //         }
                ...
                env.event_stop_record("GC23");
                float time1 = env.get_event_time("GC23"); 
                all_time += time1;
                cluster_time += time1;
                env.event_start_record("GC23");
            }
    ...
    auto stream = env.get_stream("GC23");
    env.event_start_record("GC23");
	...
	std::cout << "pre-conv Time [GC23] = " << pre_conv_time <<  "ms" <<std::endl;
	std::cout << "cluster-based conversion Time [GC23] = " << cluster_time <<  "ms" <<std::endl;
	std::cout << "post-conv Time [GC23] = " << post_conv_time <<  "ms" <<std::endl;
	std::cout << "recover Time [GC23] = " << recover_time <<  "ms" <<std::endl;
    std::cout << "GC23 info: runtime " << all_time <<  "ms" <<
